...
def sim_blocks(original_bricks):
  ...
  falling_bricks = [False for x in bricks]
  ...
  while block_fell:
    ...
    for i in range(len(bricks)):
      ...
      if can_move_down:
        block_fell = True
        falling_bricks[i] = True
        grid[grid == i+1] = 0
        bricks[i] = [(point[0], point[1], point[2] - 1) for point in bricks[i]]
        for point in bricks[i]:
          grid[point] = i + 1

  return bricks, falling_bricks

bricks, _ = sim_blocks(bricks)

shifted = 0
for i in range(len(bricks)):
  print(f'{i+1}/{len(bricks)}')
  test_bricks = bricks.copy()
  test_bricks.pop(i)
  _, fallen = sim_blocks(test_bricks)
  shifted += sum(fallen)

print('Part 2:', shifted)
